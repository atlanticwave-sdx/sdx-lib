...
class SDXValidator:
    ...
    def validate_required_attributes(
        base_url: str,
        name: Optional[str],
        endpoints: Optional[List[Dict[str, str]]],
    ) -> None:
    ...
    def validate_name(name: Optional[str]) -> Optional[str]:
        ...
        if name and (not isinstance(name, str) or len(name) > 50):
            raise ValueError(
                "Name must be a non-empty string with max 50 characters."
            )
        return name
    ...
    def validate_endpoint_dict(
        ...
        if not port_id or not isinstance(port_id, str):
            raise ValueError(
                "Each endpoint must contain a valid 'port_id' key."
            )
    ...
    def validate_endpoints(
        ...
        validated_endpoints = [
            SDXValidator.validate_endpoint_dict(ep) for ep in endpoints
        ]
        ...
        if has_vlan_range and (
            len(vlan_values) > 1 or has_special_vlan or has_single_vlan
        ):
            raise ValueError(
                "All endpoints must have the same VLAN value if one uses a range."
            )

        if has_special_vlan and (
            len(vlan_values) > 1 or has_vlan_range or has_single_vlan
        ):
            raise ValueError(
    ...
    def validate_scheduling(
        scheduling: Optional[Dict[str, str]],
    ...
    @staticmethod
    def validate_qos_metric_value(
        key: str, value_dict: Dict[str, Union[int, bool]]
    ) -> None:
        """Validates QoS metric values."""
        if "value" not in value_dict or not isinstance(
            value_dict["value"], int
        ):
            raise ValueError(f"QoS value for '{key}' must be an integer.")
        if "strict" in value_dict and not isinstance(
            value_dict["strict"], bool
        ):
            raise TypeError(
                f"'strict' in QoS metric of '{key}' must be a boolean."
            )

        valid_ranges = {
            "min_bw": (0, 100),
            "max_delay": (0, 1000),
            "max_number_oxps": (1, 100),
        }
    ...
    @staticmethod
    def validate_qos_metrics(
        qos_metrics: Optional[Dict[str, Dict[str, Union[int, bool]]]],
    ) -> None:
    ...
    def handle_http_error(
        logger: logging.Logger, e: HTTPError, operation: str
    ) -> None:
        ...
        try:
            ...
            if "application/json" in e.response.headers.get(
                ...
                try:
                    ...
                    error_details = (
                        error_json.get("error", "No detailed error provided.")
        ...
        error_message = method_messages.get(
            status_code, "Unknown error occurred."
        )
        logger.error(
            f"Failed to {operation}. Status code: {status_code}: {error_message}"
        )
